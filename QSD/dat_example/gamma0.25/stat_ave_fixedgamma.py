...
def main():
    Nss = [16,32,64,128,256,512,1024,2048,4096]
    gamma = 0.25
    dt = 0.05
...
#        print(dat_ave_dist_ee[Ns//2,1])
        ...
        plt.xlabel(r"$\mathrm{time}\times\gamma/\lambda$")
        plt.ylabel(r"$\overline{S_{[0,N_{\mathrm{s}}/2]}}/N_{\mathrm{s}}$")
        plt.plot(dat_ave_timeevol[:,1]*gamma,dat_ave_timeevol[:,4]/Ns,color=cmap(0))
        plt.fill_between(dat_ave_timeevol[:,1]*gamma,\
        ...
        plt.plot(dat_ave_timeevol[:,1]*gamma,dat_ave_timeevol[:,5]/Ns,color=cmap(0))
        plt.fill_between(dat_ave_timeevol[:,1]*gamma,\
